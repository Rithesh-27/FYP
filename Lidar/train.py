...
class savemodelcallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.check_freq == 0:
            print("************************************************************** \
                  **************************************************************")
            print("Saving new model to {}".format(self.save_path))
...
model.learn(callback=savemodelcallback(check_freq=5000),total_timesteps=25000,progress_bar=True,log_interval=1)
model.save(name)
