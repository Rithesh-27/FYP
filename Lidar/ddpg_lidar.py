...
class AirSimDroneEnv(AirSimEnv):
    ...
    def __init__(self, target_z=-20):
        ...
        self.start_position = [500, 500, 1.5]


        self.goal_position = [542, 500, 0]
        ...
        self.checkpoints = [
                ...
                # Before first walls (Obstacle 1)
                [512, 500, -1.5],

                # Through wider gap (Obstacle 2)
                [516, 502, -1.5],
                [516, 498, -1.5],
                [520, 499, -1.5],

                # Left turn region (Obstacle 3)
                [524, 500, -1.5],
                [528, 500, -1.5],
        ...
        self.observation_space = spaces.Box(
            low=0, high=1,
            shape=(1, self.state_feature_length + self.lidar_feature_length),
            dtype=np.float32
        )
    ...
    def filter_directional_points(self, points):
        ...
        for point in points:
            ...
            try:
                ...
                print("Index Error")
                print(points,point,ang = np.rad2deg(np.arctan2(point[1], point[0])))
    ...
    def obstacles_check(self,segmented_points):
        ...
        for key in segmented_points:
            if len(segmented_points[key]) == 0:
                ...
                continue
            distances=np.sqrt(np.sum(segmented_points[key][:, :-1]**2, axis=1))
    ...
    def _get_obs(self):
        ...
        state_feature = self._get_state_feature() / self.max_depth
        ...
        obstacles = (np.clip(obstacles, 0, 10) / 10)
        # print("LIDAR DATA WITH NORM",obstacles)

        feature_all = np.concatenate((obstacles, state_feature ), axis=0)
    ...
    def _get_state_feature(self):
        ...
        vertical_dist_norm = (relative_pose_z / self.max_vertical_difference / 2 + 0.5) * self.max_depth
        dist_norm = dist / self.goal_distance * self.max_depth
        ...
        state_norm = np.clip(state_norm, 0, self.max_depth)
    ...
    def is_crashed(self):
        ...
        if collision_info.has_collided or self.min_distance_to_obstacles < self.crash_distance:
            is_crashed = True
            # print("self.min_distance_to_obstacles",self.min_distance_to_obstacles)
        return is_crashed
    ...
    def cal_reward(self, done, timeout=False):
        """
        Reward function designed to prevent circling
        and enforce goal-directed navigation.
        """

        # ------------------ Constants ------------------
        REWARD_GOAL = 500.0
        REWARD_CHECKPOINT = 70.0

        PENALTY_CRASH = -100.0
        PENALTY_TIMEOUT = -30.0
        PENALTY_OUT_OF_BOUNDS = -50.0

        PROGRESS_SCALE = 10.0

        YAW_RATE_PENALTY = 0.15
        ...
        # ------------------ Distance Progress ------------------
        if self.step_num == 0:
            self.previous_dist = self.get_distance_to_goal()
        distance_now = self.get_distance_to_goal()
        progress = self.previous_dist - distance_now
        ...
        if self.min_distance_to_obstacles < 2.0:
            obstacle_cost = 1.0 - np.clip(
                (self.min_distance_to_obstacles - self.crash_distance) / 5.0,
                0.0, 1.0
            )
            reward -= OBSTACLE_PENALTY_SCALE * obstacle_cost

        # ------------------ Position Drift Penalty ------------------
        lateral_dist = self.getDis(x, y, 0, 0, goal_x, goal_y)
        position_penalty = np.clip(lateral_dist / 10.0, 0.0, 1.0)
        reward -= POSITION_PENALTY_SCALE * position_penalty

        # ------------------ Angular Velocity Penalty (Kills Circling) ------------------
        yaw_rate = abs(self.state_raw[3])  # yaw rate
        reward -= YAW_RATE_PENALTY * yaw_rate

        # ---------------- Checkpoint Reward ----------------
        reached, idx = self.check_checkpoint_reached()
        if reached:
            reward += REWARD_CHECKPOINT
            self.reached_checkpoints[idx] = True
